Feature: Marking Accounts
    ...
    Scenario: Marking an old sandbox account
        ...
        When Account Marker runs
        Then account "sandbox-tim.myerscough" should be marked for deletion

    @completed
    Scenario: A persistent account
        Given account "arbitrary-client" was created on "January 1, 2019"
        And today is "February 1, 2019"
        When Account Marker runs
        Then account "arbitrary-client" should not be marked for deletion
    ...
    Scenario: Marking a non-sandbox account for deletion
        Given I have an account "client-account"
        When I mark "client-account" for deletion
        Then I should see a notification indicating that "client-account" has been marked for deletion
